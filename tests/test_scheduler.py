...
class TestDailyRecap:
    ...
    def test_one_line_per_player_not_folded(self):
        """Each player gets their own line under a game heading.

        Pins the layout against the folded one-line-per-game variant
        that was tried and reverted for being unreadable at six
        players — see :func:`_per_game_sections`.
        """
        scores = [
            _row(1, "Alice", "queens", 714, 10),
            _row(2, "Bob", "queens", 714, 20),
        ...
        for name in ("Alice", "Bob", "Charlie"):
            assert any(
                ln.startswith(f"  {name} — ") for ln in out.splitlines()
            ), name
